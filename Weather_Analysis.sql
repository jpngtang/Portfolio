...
SELECT Weather_Data.location_id, locations.location_name
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
WHERE Weather_Data.temperature > 25 
  AND Locations.location_id IN (5, 19, 25);
...
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
WHERE Weather_Data.temperature > (SELECT AVG(temperature) FROM Weather_Data);

-- 17. Display the top 5 locations with the highest humidity levels.
SELECT Weather_Data.location_id, Weather_Data.humidity, Locations.location_name
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
...
GROUP BY Weather_Data.location_id, Locations.location_name
ORDER BY COUNT(*) DESC
LIMIT 1;

-- 20. List all locations and their respective weather conditions at the latest timestamp.
SELECT Weather_Data.location_id, Weather_Data.weather_condition, Locations.location_name
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
WHERE Weather_Data.timestamp = (SELECT MAX(timestamp) FROM Weather_Data);

-- 21. Calculate the difference between the maximum and minimum temperatures for each location.
SELECT Weather_Data.location_id, Locations.location_name, MAX(temperature) - MIN(temperature) AS temperature_difference
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
GROUP BY Weather_Data.location_id, Locations.location_name;
...
SELECT EXTRACT(MONTH FROM timestamp) AS month, ROUND(AVG(temperature),0) AS avg_temperature
FROM Weather_Data
GROUP BY EXTRACT(MONTH FROM timestamp)
...
SELECT Weather_Data.location_id, Locations.location_name, SUM(precipitation) AS total_precipitation,
...
SELECT Weather_Data.location_id, Weather_Data.wind_speed, locations.location_name
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
...
SELECT Weather_Data.location_id, locations.location_name, ROUND(AVG(temperature), 0) AS avg_temperature
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = Locations.location_id
WHERE timestamp >= CURRENT_DATE - INTERVAL '1 month'
GROUP BY Weather_Data.location_id, locations.location_name
...
SELECT Weather_Data.location_id, locations.location_name, MAX(temperature) - MIN(temperature) AS temp_variation
FROM Weather_Data
JOIN Locations ON Weather_Data.location_id = locations.location_id
GROUP BY Weather_Data.location_id, locations.location_id
...
SELECT TO_CHAR(timestamp, 'YYYY-MM-DD HH24:00:00') AS hour_slot, AVG(temperature) AS avg_temperature
FROM Weather_Data
GROUP BY Weather_Data.timestamp, DATE_TRUNC('hour', timestamp)
ORDER BY hour_slot;
